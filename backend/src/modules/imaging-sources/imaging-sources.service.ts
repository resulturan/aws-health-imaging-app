...
export class ImagingSourcesService {
    ...
    async create(createDto: CreateImagingSourceDto, userId: string) {
        ...
        if (
            !this.credentialsService.validateCredentials(createDto.credentials)
        ) {
    ...
    async update(
        ...
        if (updateDto.credentials) {
            if (
                !this.credentialsService.validateCredentials(
                    updateDto.credentials as AwsCredentials
                )
            ) {
                throw new BadRequestException("Invalid AWS credentials format");
            }
            ...
            encryptedCreds = this.credentialsService.encrypt(
                updateDto.credentials as AwsCredentials
            );
        ...
        const updated = await this.prisma.imagingSource.update({
            ...
            data: {
                ...updateData,
                ...(encryptedCreds !== source.encryptedCreds && {
                    encryptedCreds,
                }),
            },
            include: {
    ...
    async grantAccess(
        ...
        if (source.ownerId !== requesterId && userRole !== Role.ADMIN) {
            throw new ForbiddenException(
                "You do not have permission to grant access"
            );
        }
    ...
    async getDecryptedCredentials(
        ...
        if (cached) {
            return cached as AwsCredentials;
        }
    ...
    private async checkAccess(
        ...
        if (!accessGrant) {
            throw new ForbiddenException(
                "You do not have access to this imaging source"
            );
        }

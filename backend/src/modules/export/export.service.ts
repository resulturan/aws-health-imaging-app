...
export class ExportService {
  ...
  async exportStudy(
    ...
    try {
      // Get study metadata
      const metadata = await this.dicomProxyService.getMetadata(
        sourceId,
        imageSetId,
        userId,
        userRole,
      ...
      for (let i = 0; i < frameIds.length; i++) {
        ...
        try {
          const frameData = await this.dicomProxyService.getImageFrame(
            sourceId,
            imageSetId,
            frameId,
            userId,
          ...
          archive.append(frameData.imageFrameBlob, { name: fileName });
        } catch (error) {
